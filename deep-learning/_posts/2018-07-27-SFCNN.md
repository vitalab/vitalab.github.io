...
The authors' main argument in support of equivariance properties of a network is that by this, the hypothesis
 ...
 * A specific learned filter is obtained by linear combination of these basis functions using weights
   $$w_{jk} \in \mathcal{C}$$ ($$\mathcal{C}$$ denotes the complex numbers ;) ) learned by the network:
 ...
 * **Left** - The test error saturates with for more than 12 filter rotations. Also increasing the number of training
 ...
 * **Right** - Training on conventional MNIST, testing on samples rotated by the particular angle on the horizontal
    ...
    * **b**. Augmentation leads to decrease of performance also for testing on unrotated data sets for CNNs,
      but not for SFCNNs.
