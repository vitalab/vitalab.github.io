...
for folder in "${src_categories[@]}"; do
  ...
  if [ $folder == 'deep-learning' ]; then
    find ${folder}/_posts/ -name '*.md' -exec sed -i -e "/${tags_matcher}/tags:$1 ${deep_learning_tag}/" {} \;
  elif [ $folder == 'machine-learning' ]; then
    find ${folder}/_posts/ -name '*.md' -exec sed -i -e "/${tags_matcher}/tags:$1 ${machine_learning_tag}/" {} \;
  elif [ $folder == 'medical-imaging' ]; then
    find ${folder}/_posts/ -name '*.md' -exec sed -i -e "/${tags_matcher}/tags:$1 ${medical_tag}/" {} \;
  elif [ $folder == 'pruning-acceleration' ]; then
    find ${folder}/_posts/ -name '*.md' -exec sed -i -e "/${tags_matcher}/tags:$1 ${prunning_acceleration_tag}/" {} \;
  elif [ $folder == 'video-analysis' ]; then
    find ${folder}/_posts/ -name '*.md' -exec sed -i -e "/${tags_matcher}/tags:$1 ${video_analysis_tag}/" {} \;
  fi
