...
Three sources contribute to generating the prior: network output, raw edge map from the input image $hat{x}$ which obtained by Canny edge detection filter,  and a set of predefined shapes prepared $S$ by the medical expert.

<img src="/deep-learning/images/spcnn/1.png" width="800">


The contribution of shape prior term contains three-step (the orange part in fig1). First, the output of netwiork max pooled with the stride of 1 and the ‘same’ padding, then it multiplied with the raw edge image element-wise. Finally, $(gp(hat{y}\bigodot hat{x})$ The masked edge image is convolved with the shape priors in set S to generate a measurement of how
