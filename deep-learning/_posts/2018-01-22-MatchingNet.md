...
This paper proposes a solution for small data sets which suffers from overfitting. The authors employ a metric learning which helps the network to rapidly learn from small data set. They applied this idea on a small set of Image Net and they get an accuracy from 87.6% to 93.2%.


They use the advantages of one shot learning, but for prediction on the target, they investigate a metric to find how much an unseen sample is simmilar to a seen sample in a episod and this similarity can be present as a weight on classes.
