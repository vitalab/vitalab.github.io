---
layout: review
title: "AlphaGo/AlphaGoZero/AlphaZero/MuZero: Mastering games using progressively less priors"
...
- Since then, Deepmind has released three subsequent versions of the algorithm, each imposing less priors and increasing performance
...
Monte Carlo tree search (MCTS) was actually first introduced in 2006 to play the game of Go[^1] and won the 10th computer-Go tournament. It has since then been applied to many problems including chess, shogi, bridge, poker and videos games such as Starcraft.
...
where $$X_j$$ is the value of the node, $$C > 0$$ is an exploration factor, $$n$$ is the number of times the parent has been visited and $$n_j$$ the number of times the child has been visited. Division by zeroes are resolved to infinity, forcing the exploration towards yet-unexplored children nodes.
...
AlphaGo uses several networks to learn to predict the next move to make. First, a small rollout policy $$p_\pi$$ and a bigger policy $$p_\sigma$$ are trained via supervised learning to predict the winner of the game based on state representations. Even though $$p_\pi$$ only achieves ~24% accuracy and $$p_\sigma$$ ~57%, they are still useful at later steps. Once both polices are trained, $$p_\sigma$$ is used to initialize $$p_\rho$$, which is trained via self-play. Frames from $$p_\rho$$ are then used to train $$v_\theta$$, which learns a value function for states 

$$v_\theta (s) = \mathbb{E}[z_t | s_t=s, a_{t...T} \sim p_\rho]$$

with $$z$$ being the outcome of the game, $$\pm 1$$ depending on the player. $$v_\theta$$ was trained on a collection of 30 million game states, each from a different game played by versions of $$p_\rho$$ so as to provide decorelated labelled data thus reducing overfitting.
...
Above is the ELO rating for AlphaGO and European Go champion Fan Hui, as well as other Go programs and different versions of AlphaGo. AlphaGo went on to [beat Lee Sedol 4-1 afterwards](https://www.youtube.com/watch?v=WXuK6gekU1Y).

## AlphaGo Zero

In a subsequent paper, Deepmind presented AlphaGo Zero, which achieves higher performance than AlphaGo using less priors.
...
Above are results for the training on all four domains. We can see that MuZero manages to outperform or match the performances of AlphaZero on all three board games as well as exceed the performances of then-state of the art R2D2 on Atari.
